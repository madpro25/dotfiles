...
const NotificationIcon = (noti) => {
    if(!noti.image)
    return noti.app_entry;
    if (noti.image.startsWith("file://")) {
    // Remove the "file://" prefix
    let resultString = noti.image.slice(7);
    
    // Now, resultString contains the modified string without "file://"
    
    return resultString; // Display or use the modified string as needed
} else {
    // If the string doesn't start with "file://", you can handle it accordingly
    return noti.image;
}
    ...
    card_content = Box({
        ...
        children: [
            Box({
                ...
                children: [
                    ...
                    Label({
                        ...
                        wrap: true,
                        wrap_mode: Pango.WrapMode.WORD_CHAR,
                        label: content.length > 200 ? content.slice(0,200) + "..." : content,
                    }),
                ],
                setup: (self) => {
            ...
            Icon({
                ...
                hpack: "end",
                size: 64,
                icon: NotificationIcon(notification_obj)
            })
        ],
    }),
    press_cords = [0,0],
...
const holder = ({
    ...
    css: `margin-top: 50px;min-width:10px;min-height:10px;border: 0px black solid;`,
    vertical: true,
    ...
    connections: [
        [Notifications, (self,id) => {
            ...
            if (noti){
                print("noti", JSON.stringify(noti))
                self.add(
                    card({
